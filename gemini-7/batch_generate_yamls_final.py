...
def process_environment(env_path: Path, output_dir_hist: Path, output_dir_fall: Path, archive_hist_dir: Path, archive_list_dir: Path, conda_exe: str, use_shell: bool, use_original_name: bool) -> tuple[str, list]:
    ...
    if final_yaml_data:
        # <<< MODIFICATION: Choose output directory based on method used >>>
...
if __name__ == "__main__":
    ...
    # <<< MODIFICATION: Define separate final output directories >>>
    output_dir_main = Path(args.output_dir).resolve()
    output_dir_history_ok = output_dir_main / "from_history" # For good history YAMLs
    output_dir_fallback = output_dir_main / "from_fallback" # For fallback YAMLs
    ...
    try:
        ...
        output_dir_history_ok.mkdir(parents=True, exist_ok=True)
        output_dir_fallback.mkdir(parents=True, exist_ok=True)
    ...
    if found_envs:
        ...
        for env_path in sorted(found_envs):
            try:
                # <<< MODIFICATION: Pass both final output directories to the function >>>
                status, notes = process_environment(
                    env_path,
                    output_dir_history_ok, # Pass the "good history" dir
                    output_dir_fallback,   # Pass the "fallback" dir
                    archive_dir_history,
                    archive_dir_list,
                    str(conda_exe_path),
                    args.use_shell,
        ...
        # (Final summary block remains the same)
