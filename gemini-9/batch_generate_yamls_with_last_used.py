...
def process_environment(env_path: Path, output_dir_hist: Path, output_dir_fall: Path, archive_hist_dir: Path, archive_list_dir: Path, conda_exe: str, use_shell: bool, use_original_name: bool) -> tuple[str, list]:
    ...
    final_yaml_data = None; method_used = "unknown"; recap_notes = []
    safe_filename_base = path2str(env_path).replace(os.path.sep, '_').strip('_'); original_env_name = env_path.name
    try:
        ...
        log.error(f"  ERROR getting or parsing history for {env_path}. Skipping. Error: {e}"); return f"ERROR: Failed to get/parse history - {e}", []
    pip_section = None
    try:
    ...
    if history_is_good:
        method_used = "History"; log.info(f"  Processing using good {method_used} data.")
        # (Method 1 logic...)
        try:
        ...
        method_used = "Fallback (Filtered List)"; log.info(f"  Falling back to filtered 'conda list --export' method. Reason: {reason}")
        # (Method 2 logic...)
        try:
...
def get_latest_mtime_in_dir(dir_path: Path) -> datetime | None:
    """Recursively finds the most recent file modification time in a directory."""
    latest_mtime = 0
    try:
        ...
        log.debug(f"Could not scan directory {dir_path} for mtime: {e}")
        return None

    return datetime.fromtimestamp(latest_mtime) if latest_mtime > 0 else None

# --- MODIFIED find_conda_environments function ---
        ...
        for root, dirs, files in os.walk(base_path, topdown=True, onerror=lambda err: log.error(f"  Permission error accessing {err.filename} - Skipping subtree.")):
            if 'conda-meta' in dirs:
                ...
                if (env_path / 'conda-meta' / 'history').exists() and not (env_path / 'pkgs').is_dir():
                    ...
                    # <<< MODIFICATION: Get last modified date here >>>
...
if __name__ == "__main__":
    ...
    if found_envs_data:
        ...
        # <<< MODIFICATION: Sort by last modified date, oldest first >>>
        # Environments with unknown dates will be at the end
        found_envs_data.sort(key=lambda x: x[1] if x[1] else datetime.max)

        for env_path, last_modified in found_envs_data:
            try:
                # Pass the necessary directories to the processing function
                ...
                # <<< MODIFICATION: Store last_modified date in recap data >>>
                recap_data[str(env_path)] = (status, notes, last_modified)
            except Exception as e:
                log.error(f"CRITICAL ERROR processing {env_path}. Error: {e}"); recap_data[str(env_path)] = ("CRITICAL ERROR", [str(e)], None)
        
        # --- MODIFIED FINAL SUMMARY BLOCK ---
        log.info("\n\n" + "="*80); log.info("FINAL PROCESSING SUMMARY"); log.info("="*80)
        total_envs = len(recap_data); ok_count = sum(1 for status, _, _ in recap_data.values() if status.startswith("OK")); fallback_count = sum(1 for status, _, _ in recap_data.values() if "Fallback" in status); history_ok_count = ok_count - fallback_count; error_count = total_envs - ok_count
        ...
        if ok_count > 0:
            ...
            for env_path_str, (status, notes, last_mod) in recap_data.items():
                if not status.startswith("OK"): continue
                notes_str = f" | Notes: {'; '.join(notes)}" if notes else ""
                last_mod_str = last_mod.strftime('%Y-%m-%d') if last_mod else "Unknown"
                log.info(f"  - Env: {env_path_str}")
                log.info(f"    - Last Modified: {last_mod_str:<15} | Status: {status}{notes_str}")
        if error_count > 0:
