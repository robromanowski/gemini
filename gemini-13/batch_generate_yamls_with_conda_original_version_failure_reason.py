...
def process_environment(env_path: Path, output_dir_hist: Path, output_dir_fall: Path, archive_hist_dir: Path, archive_list_dir: Path, conda_exe: str, use_shell: bool, use_original_name: bool) -> dict:
    ...
    if history_is_good:
        ...
        result['failure_reason'] = reason # <<< ADDED
    ...
    if final_yaml_data:
        ...
        try:
            with open(output_filepath, 'w', encoding='utf-8') as f: yaml.dump(final_yaml_data, f, default_flow_style=False, sort_keys=False)
...
def write_summary_csv(results: list, output_filepath: Path):
    ...
    fieldnames = [
        ...
        "creation_conda_version", "conda_version", "status", "method",
        "failure_reason", # <<< ADDED
...
if __name__ == "__main__":
    ...
    if found_envs_data:
        ...
        for env_path, last_modified in found_envs_data:
            try:
                ...
                all_results.append({
                    ...
                    "filtered_list": "", "notes": str(e), "failure_reason": "Critical Error"
                })
        ...
        for r in all_results:
            ...
            reason_str = f" (Reason: {r['failure_reason']})" if r.get('failure_reason') != 'N/A' else ""
            status_str = f"Status: {r['status']} ({r['method']}{reason_str})"
