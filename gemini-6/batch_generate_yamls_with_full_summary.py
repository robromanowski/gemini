...
def process_environment(env_path: Path, output_dir: Path, archive_hist_dir: Path, archive_list_dir: Path, conda_exe: str, use_shell: bool, use_original_name: bool) -> tuple[str, list]:
    ...
    try:
        ...
        warning_msg = f"  WARNING: Failed to get/process pip deps separately for {env_path}. Error: {e}"
        log.warning(warning_msg)
        recap_notes.append("Pip processing failed")

    history_is_good, reason = is_history_output_good(hist_data)
    if history_is_good:
        method_used = "History"
        log.info(f"  Processing using good {method_used} data.")
        # ... (Method 1 logic remains the same) ...
        try:
        ...
        # ... (Method 2 logic remains the same) ...
        try:
    ...
    # --- Save FINAL Output YAML and return status ---
    if final_yaml_data:
...
if __name__ == "__main__":
    ...
    log.setLevel(args.log_level); logging.getLogger().setLevel(args.log_level)

    # (Directory creation and conda executable resolution remain the same)
    output_dir_main = Path(args.output_dir).resolve(); archive_dir_history = output_dir_main / "raw_history_outputs"; archive_dir_list = output_dir_main / "raw_list_export_outputs"
    try:
        output_dir_main.mkdir(parents=True, exist_ok=True); archive_dir_history.mkdir(parents=True, exist_ok=True); archive_dir_list.mkdir(parents=True, exist_ok=True)
        log.info(f"Main output directory: {output_dir_main}"); log.info(f"Raw history archive: {archive_dir_history}"); log.info(f"Raw list export archive: {archive_dir_list}")
    ...
    # (Environment discovery remains the same)
    log.info("Starting environment discovery..."); search_paths_str = [str(p) for p in args.search_paths]; found_envs = find_conda_environments(search_paths_str)
    log.info(f"Discovery finished. Found {len(found_envs)} potential environments.")

    if found_envs:
        log.info("Starting environment processing...")
        recap_data = {} # Dictionary to hold recap info for each environment

        for env_path in sorted(found_envs):
            try:
                status, notes = process_environment(env_path, output_dir_main, archive_dir_history, archive_dir_list, str(conda_exe_path), args.use_shell, args.use_original_name)
        ...
        total_envs = len(recap_data)
        ok_count = sum(1 for status, notes in recap_data.values() if status.startswith("OK"))
        fallback_count = sum(1 for status, notes in recap_data.values() if "Fallback" in status)
        history_ok_count = sum(1 for status, notes in recap_data.values() if status == "OK (History)") # <<< NEW COUNTER
        error_count = total_envs - ok_count

        log.info(f"Total Environments Processed: {total_envs}")
        log.info(f"  Successfully Generated: {ok_count}")
        log.info(f"  Errors / Not Generated: {error_count}")
        log.info(f"  --- Breakdown of Successes ---")
        log.info(f"    Processed via Clean History: {history_ok_count}") # <<< NEW LOG LINE
        log.info(f"    Processed via Fallback Method: {fallback_count}")
        log.info("-"*80)

        if ok_count > 0:
            log.info("Breakdown by Environment:")
            for env_path_str, (status, notes) in recap_data.items():
                if not status.startswith("OK"): continue # Skip errors here
                notes_str = f" | Notes: {'; '.join(notes)}" if notes else ""
                log.info(f"  - Env: {env_path_str}")
                log.info(f"    - Status: {status}{notes_str}")
        
        if error_count > 0:
            log.info("\n--- Environments with Errors ---")
            for env_path_str, (status, notes) in recap_data.items():
                if not status.startswith("OK"):
                    notes_str = f" | Details: {'; '.join(notes)}" if notes else ""
                    log.info(f"  - Env: {env_path_str}")
                    log.info(f"    - Status: {status}{notes_str}")
