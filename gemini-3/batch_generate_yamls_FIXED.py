...
def run_conda_command(conda_exe_path: str, args_list: list, env_path: Path = None, use_shell: bool = False):
    # Runs a conda command using subprocess.
...
def get_pip_deps_from_export(export_yaml_data: dict) -> dict | None:
    # Extracts/cleans pip deps from parsed export data.
...
def parse_conda_list_export(export_output: str) -> list:
    # Parses 'conda list --export' output (package=version=build -> package=version).
    dependencies = []
...
def is_history_output_good(hist_data: dict) -> bool:
    """
    Heuristic check if --from-history output looks minimal and clean.
    NOW MODIFIED to also check for known low-level packages.
    """
...
def process_environment(
    ...
    conda_exe: str,
    use_shell: bool,
    use_original_name: bool    # <<< New flag argument
    ):
    """
    Generates the lean YAML, choosing method based on history, saves raw outputs,
    and optionally uses the original environment name.
    """
    ...
    original_env_name = env_path.name # Get original name from path
    ...
    if is_history_output_good(hist_data):
        ...
        try:
            ...
            new_deps = []
            if original_python_spec: new_deps.append(original_python_spec)
            new_deps.extend(["pip", "setuptools", "wheel"]) # Add core unconditionally
...
# --- find_conda_environments function (from your original script) ---
def find_conda_environments(search_paths: list) -> list:
    ...
    for base_path_str in search_paths:
        ...
        if not base_path.is_dir():
            ...
            continue
        # Using os.walk for better control over recursion and error handling
        for root, dirs, files in os.walk(base_path, topdown=True, onerror=lambda err: log.error(f"  Permission error accessing {err.filename} - Skipping subtree.")):
            if 'conda-meta' in dirs:
                env_path = Path(root)
                # Check for history file and ensure it's not a 'pkgs' cache dir
                if (env_path / 'conda-meta' / 'history').exists() and not (env_path / 'pkgs').is_dir():
                    ...
                    dirs[:] = [] # Don't descend further into this env directory
                else:
                    log.debug(f"  Skipping {env_path}, doesn't look like a standard named/prefix env or history missing.")
                    dirs[:] = [] # Don't descend further
            # Prune common non-environment directories for efficiency
