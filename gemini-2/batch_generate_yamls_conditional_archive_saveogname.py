...
# packaging library not strictly needed now, but harmless to import
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Batch generate conda environment YAMLs targeting conda-forge, using history if reliable, "
                    ...
                    "archives raw outputs, and optionally uses original env name.", # Updated description
         ...
         for env_path in sorted(found_envs):
              try:
                  process_environment( # Main processing function
                      ...
                      str(conda_exe_path),
                      args.use_shell,
                      args.use_original_name # <<< Pass the new flag value
                  )
                  success_count += 1
              except Exception as e:
