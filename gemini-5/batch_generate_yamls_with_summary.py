...
def process_environment(env_path: Path, output_dir: Path, archive_hist_dir: Path, archive_list_dir: Path, conda_exe: str, use_shell: bool, use_original_name: bool):
    ...
    if history_is_good:
        ...
        try:
            ...
            err_msg = f"  ERROR processing good history for {env_path}. Error: {e}"
            log.error(err_msg)
            final_yaml_data = None
            recap_notes.append(f"ERROR: {err_msg}")
    else:
        ...
        try:
            # (Full logic for METHOD 2 as you provided)
            # ... (omitted for brevity, this part is unchanged) ...
            ...
            for dep in dependencies_from_list:
                ...
                if package_name in FILTER_OUT_PACKAGES:
                    filtered_count += 1
                    filtered_packages_list.append(dep)
                else:
                    new_deps.append(dep)
                    kept_count += 1
            ...
            if kept_count == 0:
                recap_notes.append("Kept 0 packages")
            if pip_section: new_deps.append(pip_section)
            if use_original_name:
                final_env_name = original_env_name
            else:
            ...
            err_msg = f"  ERROR processing with fallback method for {env_path}. Error: {e}"
            log.error(err_msg)
            final_yaml_data = None
            recap_notes.append(f"ERROR: {err_msg}")
    ...
    if final_yaml_data:
        ...
        try:
            ...
            err_msg = f"  ERROR writing final YAML file {output_filepath}. Error: {e}"
            log.error(err_msg)
...
if __name__ == "__main__":
    ...
    # (Conda executable resolution remains the same)
    ...
    if found_envs:
        ...
        # <<< MODIFICATION FOR SUMMARY STARTS HERE >>>
        recap_data = {} # Dictionary to hold recap info for each environment

        for env_path in sorted(found_envs):
            try:
                # process_environment will now return a status and notes
                status, notes = process_environment(
                    env_path,
                    output_dir_main,
                    archive_dir_history,
                    archive_dir_list,
                ...
                recap_data[str(env_path)] = (status, notes) # Store recap info
            except Exception as e:
                log.error(f"CRITICAL ERROR processing {env_path}. Error: {e}")
                recap_data[str(env_path)] = ("CRITICAL ERROR", [str(e)])
        ...
        log.info(f"  Environments Using Fallback Method: {fallback_count}")
        log.info("-"*80)
        ...
        log.info("="*80)
        # <<< MODIFICATION FOR SUMMARY ENDS HERE >>>
    else:
